...
def draw(draw_info, algo_name, ascending):
    ...
    sorting = draw_info.FONT.render("I - Insertion | B - Bubble | S - Selection | Q - Quick | M - Merge", 1, draw_info.BLACK)
...
def quick_sort(draw_info, ascending=True):
    ...
    def quick_sort_helper(start, end):
        ...
        draw_list(draw_info, {partition_idx: draw_info.GREEN, end: draw_info.RED}, True)
        yield True

        yield from quick_sort_helper(start, partition_idx - 1)
        yield from quick_sort_helper(partition_idx + 1, end)
...
def merge_sort(draw_info, ascending=True):
    ...
    def merge_sort_helper(start, end):
        ...
        for i in range(start, end + 1):
            if l_idx < len(left) and (r_idx >= len(right) or (left[l_idx] < right[r_idx] if ascending else left[l_idx] > right[r_idx])):
                lst[i] = left[l_idx]
                l_idx += 1
            else:
                lst[i] = right[r_idx]
